...
parser.add_argument('--hidden', type= int, default = 16, help ='hidden size of GCN')
parser.add_argument('--dropout',type= float, default= 0.5, help= 'dropout of GCN')
...
adj = sparse_to_torch_sparse(sp_adj, device=args.device)                      #将adj变成torch sparse tensor adj
...
if __name__ == '__main__':
    ...
    if not args.preprocessed:
        adj_dense = sparse_to_torch_dense(sp_adj, device='cpu')              #将adj变成dense的adj
        #feat_dict, precompute_time = sgc_precompute(adj, adj_dense, args.degree-1, index_dict, args.combine)
        #feat_dict, precompute_time = sjk_precompute(adj, adj_dense, args.degree-1, index_dict, args.combine)
        feat_dict, precompute_time = spr_precompute(adj, adj_dense, args.degree-1, index_dict, args.combine)
    else:
    ...
    model = SGC(nfeat=feat_dict["train"].size(1), nclass=nclass, degree = args.degree, combine = args.combine)

    #model = GCN(nfeat=feat_dict["train"].size(1), nhid= args.hidden, nclass = nclass, dropout = args.dropout)
